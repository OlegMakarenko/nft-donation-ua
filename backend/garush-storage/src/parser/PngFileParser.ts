...
export class PngFileParser implements FileParser {
    ...
    private static TYPE_pHYs = 0x70485973;
    private static TYPES = [this.TYPE_IHDR, this.TYPE_IDAT, this.TYPE_IEND, this.TYPE_PLTE, this.TYPE_tRNS, this.TYPE_gAMA, this.TYPE_pHYs];
    ...
    public getChunk(data: Buffer, start: number, expectedType?: number): PngChunk {
        ...
        console.log(`png type ${Convert.uint8ToHex(chunkType)} |  name = ${name} | size = ${chunkSize}`)
        const isValid = PngFileParser.TYPES.find((t) => t == type);
        if (!isValid) {
            throw Error(`Invalid png type ${Convert.uint8ToHex(chunkType)}`);
        }
